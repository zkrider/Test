...
def getManagerData(nsmapi):
    ...
    yields error "/GUIAccess/PasswordControl"
    yields error "/GUIAccess/sessioncontrol"
    yields error "/loginhistory"
    ...
    "/Maintenance/BackupNow",
    "/malwarearchive/list",
    # "/nsm", only gives NSM version number
    "/alerts/TopN/active_botnets",
    "/alerts/TopN/attack_applications",
    "/alerts/TopN/attack_subcategories",
    "/alerts/TopN/attacker_countries",
    ...
    for resource in resources:
        url = resource
        data[resource.replace("/", "_")[1:]] = getData(nsmapi, url, resource, time.time())

    flatData = pd.json_normalize(data).transpose().to_csv()
